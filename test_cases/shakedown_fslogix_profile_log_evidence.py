...
def run(ctx):
    ...
    try:
        ...
        ctx.step(f"Step 3: Open FSLogix Profile logs folder: {FSLOGIX_PROFILE_LOG_PATH}")
        ctx.hotkey("winleft", "r")
        ctx.wait(ctx.config.wait("run_dialog_wait_sec", 1.5))
        ctx.type_text(FSLOGIX_PROFILE_LOG_PATH, interval=0.15)
        ctx.press("enter")
        ctx.wait(ctx.config.wait("fslogix_profile_logs_open_wait_sec", 5.0))
        explorer_opened = True

        ctx.step("Step 4: Select first profile log file with Down Arrow, then open it")
        ctx.press("down")
        ctx.wait(ctx.config.wait("fslogix_log_select_wait_sec", 0.5))
        ctx.press("enter")
        ctx.wait(ctx.config.wait("fslogix_notepad_open_wait_sec", 2.0))
        notepad_opened = True

        ctx.step("Step 5: Maximize Notepad window")
        ctx.maximize_active_window()
        ctx.wait(ctx.config.wait("fslogix_notepad_after_maximize_wait_sec", 1.0))

        ctx.step(f"Step 6: Search FSLogix Profile log for: {SEARCH_TERM}")
        ctx.hotkey("ctrl", "f")
        ...
        ctx.step("Step 7: Capture FSLogix Profile log evidence screenshot and copy it to clipboard")
        ...
        if notepad_opened:
            ctx.step("Step 8: Close Notepad with Alt + F4")
            ctx.hotkey("alt", "f4")
            ctx.wait(ctx.config.wait("fslogix_notepad_close_wait_sec", 1.0))
        if explorer_opened:
            ctx.step("Step 9: Close File Explorer with Alt + F4")
            ctx.hotkey("alt", "f4")
            ctx.wait(ctx.config.wait("fslogix_explorer_close_wait_sec", 1.0))
